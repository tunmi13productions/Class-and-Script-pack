...
class sound_pool_item:
 def __init__(self,filename, **kwargbs):
  ...
  self.encrypted=kwargbs.get("encrypted",False)
 def reset(self,pack=""):
  self.__init__("")
 def update(self,listener_x,listener_y,listener_z,max_distance):
  if max_distance>0 and self.looping:
   ...
   if total_distance>max_distance and self.handle.handle!=None:
    ...
    return
   if total_distance<=max_distance and self.handle.handle==None:
    try: self.handle.load(self.packname+self.filename)
    except:
     pass
     return
...
class sound_pool:
 def __init__(self,packname="",encrypted=False):
  self.items=[]
 ...
 def play_stationary_extended(self,filename, looping, offset=0, start_pan=0, start_volume=0, start_pitch=100, persistent=False):
  self.clean_unused()
  s=sound_pool_item(filename=filename,looping=looping,start_offset=offset,start_pan=start_pan,start_volume=start_volume,start_pitch=start_pitch,persistent=persistent,stationary=True,encrypted=self.encrypted)
  try: s.handle.load(self.packname+filename)
  except:
   s.reset()
   return -1 
 ...
 def resume_sound(self,s):
  ...
  if s.handle.handle!=None and not s.handle.handle.is_playing:
 ...
 def set_pack_name(self,pack):
  self.packname=pack
 def toggle_encryption(self,enc):
  self.encrypted=enc
 def clean_unused(self):
  ...
  for i in self.items:
   ...
   if i.handle.handle==None or not i.handle.handle.is_playing and not i.paused: self.items.remove(i)
p=sound_pool()