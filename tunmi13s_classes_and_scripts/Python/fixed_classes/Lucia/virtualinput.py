...
"""
from gamelib.auto import speak
import lucia
import string
...
class VirtualInput:
  ...
  #Determines whether the run function should break out of it's loop
	...
	def toggle_character_repetition(self):
		self.repeating_characters=not self.repeating_characters
	...
	def translate_character(self,char):
		...
		char.replace("^","caret")
		char.replace("&","ampersand")
		char.replace("*","star")
		char.replace("(","left paren")
		char.replace(")","right paren")
		return char
	...
	def get_integer_list_input(self, messages, callbacks = [], negative = False, decimal = False):
		...
		return results
def ctrldown():
	if lucia.key_down(lucia.K_LCTRL) or lucia.key_down(lucia.K_RCTRL): return True
	return False
